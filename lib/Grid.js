...
function isValidDirection(line) {
  ...
  return Math.abs(line.start.y - line.end.y) === Math.abs(line.start.x - line.end.x);
}
...
function isLineWithinGrid(line, grid) {
  return isPointWithinGrid(line.start, grid) && isPointWithinGrid(line.end, grid);
}
...
Grid.prototype.forEachUnusedLetterInLine = function (line, action) {
  var self = this;
  forEachLetterInLine(line, function (cursor) {
    if(!self.getLetter(cursor.x, cursor.y).used) {
      action(cursor);
    }
  });
};
...
Grid.prototype.getLetters = function (line) {
  ...
  forEachLetterInLine(line, function (cursor) {
    letters.push(self.getLetter(cursor.x, cursor.y));
  });
...
Grid.prototype.markUnusedLettersUsed = function (line, playerId) {
  var self = this;

  this.forEachUnusedLetterInLine(line, function (cursor) {
    self.emit('letterUsed', {
      location: cursor,
      player: playerId
    });
  });
