  ...
  this.httpServer.listen(function () {
    self.httpServer.on('newPlayer', function (playerSocket) {
      ...
      playerSocket.on('setName', function (newName) {
        playerSocket.emit('nameChanged', newName);
      });

      var id = self.idFactory();
      self.eventStore.push({event: 'newPlayer', data: id});
      self.httpServer.emitToAllPlayers('newPlayer', id);
    });

    self.grid.on('letterUsed', function (letterPos) {
      self.eventStore.push({event: 'letterUsed', data: letterPos});
      self.httpServer.emitToAllPlayers('letterUsed', letterPos);
    });
    callback();
  });
};
